...
pass(pkg.version==='2.0.0-rc.1','E12 package identity must be 2.0.0-rc.1');
...
if(existsSync(join(root,'src/app/e12Certification.ts'))){
  ...
  pass(certification.includes("E12_TARGET_VERSION = '2.0.0-rc.1'"),'E12 target version drifted');
  pass(certification.includes('E12_CERTIFICATION_DOMAINS = 22'),'E12 fixed rubric must remain 22 domains');
  for(const gate of ['golden-cross-domain-corpus','catalog-and-capability-consistency','exactness-provenance','pwa-static-contract'])pass(certification.includes(gate),`E12 automated gate missing: ${gate}`);
  for(const gate of ['Android Chrome smoke','iOS Safari/WebKit smoke','real deployed PWA install/offline/upgrade cycle'])pass(certification.includes(gate),`E12 external release gate missing: ${gate}`);
}
...
if(existsSync(join(root,'docs/RELEASE_CERTIFICATION.md'))){
  ...
  pass(release.includes('v2.0.0-rc.1'),'release certification must identify v2.0.0-rc.1');
  pass(release.includes('NOT CERTIFIED')||release.includes('not certified'),'release certification must explicitly withhold stable v2.0.0');
}
